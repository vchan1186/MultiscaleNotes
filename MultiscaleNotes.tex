...
The virtual work, $\delta W$, is obtained by rewritting Eq.\ \eqref{eq:macro_stress_divergence} in a residual form, $\pmb{r}$, and multiplying by a virtual velocity, $\delta \pmb{v}$ (also known as a test function) \cite{JavierBonet:2008uxa}
%
\begin{align}
...
\end{equation}
%
Since the virtual work is expressed in the current configuration (c.f., Eq.\ \eqref{eq:virtual_work}), linearization is not straightforward because the integration bound changes with motion. Therefore, to move forward with the linearization procedure, we must first linearize the virtual work expressed in the reference configuration and then  employ a push-forward operation to obtain the linearization for the spatial description of Eq.\ \eqref{eq:virtual_work}. \red{Note that the form of the terms involving $\pmb{t}$ and $\pmb{Q}$ in Eq.\ \eqref{eq:virtual_work} do not change upon linearization because $\pmb{t}$ and $\pmb{Q}$ are assumed to not change with motion. Therefore, from this point on, only the linearization of internal virtual work, $\delta W_{int}$, is considered.} 

The internal virtual work (c.f., Eq.\ \eqref{eq:virtual_work_final}) is
%
\begin{equation}
\delta W_{int}(\phi,\delta \pmb{v}^{(M)}) = \int_{V_0^{(M)}} \left((\pmb{\sigma}^{(M)}:\delta \pmb{d}^{(M)}) J + (\pmb{\sigma}^{(M)}\delta \pmb{v}^{(M)})\cdot \nabla J \right)dV_0^{(M)}
\end{equation}
%
% begin comment
...
%
\begin{eqnarray}
D\delta W(\phi,\delta \pmb{v})[\delta \pmb{u}]  
%
...
%
\begin{align}
&\pmb{u} = \sum_{a=1}^n N_a \pmb{x}_a \nonumber\\
%
&\delta \pmb{v} = \sum_{a=1}^n N_a \delta \pmb{v}_a \nonumber\\
%
&\delta \pmb{d} = \frac{1}{2} \sum_{a=1}^n (\delta \pmb{v}_a \otimes \nabla N_a + \nabla N_a \otimes \delta \pmb{v}_a) \nonumber\\
%
&\delta \pmb{\varepsilon} = \frac{1}{2} \sum_{a=1}^n (\pmb{u}_a \otimes \nabla N_a + \nabla N_a \otimes \pmb{u}_a)
\end{align}
%
